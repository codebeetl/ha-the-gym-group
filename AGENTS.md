...
Keep the minimum Home Assistant version aligned with `hacs.json` (currently 2026.3.0). This is a cloud-polling integration: only HTTPS `netpulse.com` hosts accepted by `is_valid_host()` may be used. Validate stored configuration again during entry setup. Diagnostics must redact usernames, passwords, entry IDs, and timestamps; credentials must never be logged.
